...
public class Server {
	...
	/**
	 * Constructor. \n The persiscanceManager is initialices as well as the
	 * Transaction
	 */
	public Server() {
	...
	/**
	 * STORE EXPENSE\n The idea is to store the expense and the relationship between
	 * the user\n This method is called from the
	 * es.deusto.client.ExampleClient.storeExpense class which receives the info
	 * from the es.deusto.client.AddExpenseWindow\n
	 * 
	 * @param directedExpense - is the object that stores both the user and the
	 *                        expense
	...
	public Response storeExpense(DirectedMessage directedExpense) {
		...
		try {
			...
			if (user != null) {
				Expense expense = new Expense(directedExpense.getExpenseData().getText(),
						directedExpense.getExpenseData().getAmount(), directedExpense.getExpenseData().getCategory());
	...
	/**
	 * VALIDATE USER.\n This method access the DB and searches if the given user is
	 * already stored\n This method is called from the client side class
	 * es.deusto.client.ExampleClient and the data is recieved from the
	 * es.deusto.client.LoginWindow\n
	 * 
	 ...
	 * called in the es.deusto.client.ExampleClient class and the data of the user
	 * is obtain in the es.deusto.client.RegisterWindow\n
	 * 
	 * @param userData
